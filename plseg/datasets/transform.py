...
class ClassUniform(object):
    def __init__(self, img_size=(1024, 1024), crop_size=512, crop_nopad=True,
                 scale_min=0.75, scale_max=1.25, ignore_index=12, class_list=[2, 3]):
        """
    ...
    def __call__(self, img, mask):
        ...
        if scale_amt < 1.0:
            ...
            sum_arr = np.zeros(self.img_size).astype('float32')
            tmp = np.zeros(self.img_size).astype('float32')
