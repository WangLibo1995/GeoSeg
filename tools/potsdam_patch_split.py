...
def parse_args():
    ...
    parser.add_argument("--img-dir", default="data/postdam/train_images")
    parser.add_argument("--mask-dir", default="data/postdam/train_masks")
    parser.add_argument("--output-img-dir", default="data/postdam/train/images")
    parser.add_argument("--output-mask-dir", default="data/postdam/train/masks")
    parser.add_argument("--eroded", action='store_true')
    parser.add_argument("--gt", action='store_true')  # output RGB mask
    parser.add_argument("--rgb-image", action='store_true')  # choose Potsdam RGB images, if not using IRRG images
...
def image_augment(image, mask, patch_size, gt, mode='train', val_scale=1.0):
    image_list = []
    mask_list = []
...
def patch_format(inp):
    ...
    image_list, mask_list = image_augment(image=img.copy(), mask=mask.copy(), patch_size=split_size,
                                          val_scale=val_scale, mode=mode, gt=gt)
